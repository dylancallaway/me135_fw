...
void interrupt_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)interrupt__INTC_NUMBER] = address;
}
...
cyisraddress interrupt_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)interrupt__INTC_NUMBER];
}
...
{
    *interrupt_INTC_PRIOR = priority << 5;
}
...
uint8 interrupt_GetPriority(void)
{
    ...
    priority = *interrupt_INTC_PRIOR >> 5;
